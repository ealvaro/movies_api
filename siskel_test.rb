...
class TestSiskel < Minitest::Test
  def test_movie_title
    ...
    assert_equal 'Kill Bill: Volume 1', movie.title
  end

  def test_movie_rating
    movie = Siskel.new('The Hangover')
    assert_equal 'R', movie.rated
  end
  ...
  def test_remake_with_different_year
    movie = Siskel.new('Parent Trap', y: 1961)
    assert_equal '1961', movie.year
  end

  def test_movie_with_full_plot
    movie = Siskel.new('Titanic', plot: :full)
    assert_equal '84 years later, a 100 year-old woman named Rose DeWitt Bukater tells the story to her granddaughter Lizzy Calvert, Brock Lovett, Lewis Bodine, Bobby Buell and Anatoly Mikailavich on the Keldysh about her life set in April 10th 1912, on a ship called Titanic when young Rose boards the departing ship with the upper-class passengers and her mother, Ruth DeWitt Bukater, and her fiancé, Caledon Hockley. Meanwhile, a drifter and artist named Jack Dawson and his best friend Fabrizio De Rossi win third-class tickets to the ship in a game. And she explains the whole story from departure until the death of Titanic on its first and last voyage April 15th, 1912 at 2:20 in the morning.', movie.plot
  end
  ...
  def test_movie_consensus_negative
    movie = Siskel.new('Terror Trail')
    assert_equal '6.4', movie.imdbrating
  end

  def test_movie_consensus_positive
    movie = Siskel.new('Daredevil')
    assert_equal '8.6', movie.imdbrating
  end
end
